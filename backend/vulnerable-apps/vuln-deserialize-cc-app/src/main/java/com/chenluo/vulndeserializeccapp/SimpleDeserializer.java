package com.chenluo.vulndeserializeccapp;

import java.io.ByteArrayInputStream;
import java.io.ObjectInputStream;
import java.util.Base64;

/**
 * Standalone utility that decodes and deserializes an embedded Base64 payload.
 *
 * <p>This is used as a minimal local reproduction helper for the deserialization lab.
 */
public class SimpleDeserializer {

    /**
     * Runs one decode + deserialize attempt.
     *
     * @param args process arguments
     * @throws Exception when decode/deserialization fails
     */
    public static void main(String[] args) throws Exception {
        String b64payload = "";

        System.out.println("Payload received. Decoding...");
        byte[] serializedData = Base64.getDecoder().decode(b64payload);
        System.out.println("Decoding complete. Deserializing...");

        ByteArrayInputStream bais = new ByteArrayInputStream(serializedData);
        ObjectInputStream ois = new ObjectInputStream(bais);
        Object obj = ois.readObject();
        ois.close();

        System.out.println("Deserialization finished. Object type: " + obj.getClass().getName());
    }
}
