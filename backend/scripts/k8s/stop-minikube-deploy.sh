...
#
# Teardown call chain (high level):
# 1) Remove dynamically created lab resources by label.
# 2) Remove static platform resources (ingress/services/deployments/rbac).
# 3) Stop minikube tunnel.
# 4) Optionally stop or delete minikube.
...
echo -e "${YELLOW}Deleting gateway-service...${NC}"
kubectl delete service gateway-service-svc -n default
kubectl delete deployment gateway-service-deployment -n default
check_status "gateway-service deletion"

# Delete auth-service
echo -e "${YELLOW}Deleting auth-service...${NC}"
kubectl delete service auth-service-svc -n default
kubectl delete deployment auth-service-deployment -n default
check_status "auth-service deletion"
